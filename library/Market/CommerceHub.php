...
class CommerceHub extends StoreInterface
{
    protected $store_lookup = ['jcpenney' => 'jcp-01',
        'staplesus' => 'staples-01',
        'staplesadvus' => 'staples-02',
        'staplesquill' => 'staples-03',
        'staplesca' => 'staples-04',
        'macys' => 'macys-01'
    ];

    protected $dir = ['jcp-01' => '/EDI/JCPenney/',
        'staples-01' => '/EDI/Staples/',
        'staples-02' => '/EDI/Staples/',
        'staples-03' => '/EDI/Staples/',
        'staples-04' => '/EDI/Staples/',
        'macys-01' => '/EDI/Macys/'
    ];

    protected $shipping = [
        'jcp-01' => ['carrier' => 'UP', 'method' => 'S_GROUND'],
        'macys-01' => ['carrier' => 'UP', 'method' => 'S_GROUND'],
    ];
        ...
        foreach ($data as $line) {
            ...
            if ($line[0] != 'PKG_PACKAGE_ID') {
                ...
                if ($line[15] != $po) {
                    ...
                    Log::info('CommerceHub import: Processing order ' . $line[15]);
    ...
    private function insertOrder($data, $store)
    {
        ...
        if(array_key_exists($store->store_id, $this->shipping)) {
            $order->carrier = $this->shipping[$store->store_id]['carrier']; //??
            $order->method = $this->shipping[$store->store_id]['method'];
        } else {
            $order->carrier = '';
            $order->method = '';
        }
    ...
    public function exportShipments($store, $shipments)
    {
        ...
        Log::info('CommerceHub shipment csv started');
        ...
        if (count($lines) > 0) {
            ...
            try {
                ...
                // copy($path . $filename, storage_path() . $this->dir[$store] . $filename);
            } catch (\Exception $e) {
                Log::error('Error Creating CommerceHub CSV - ' . $e->getMessage());
                return;
            }
        }

        Log::info('CommerceHub shipment csv upload created');
